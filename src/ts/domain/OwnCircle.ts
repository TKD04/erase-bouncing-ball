...
export default class OwnCircle {
  ...
  moveToUp(): void {
    ...
    if (this.#WillOverBilliardsTableTop()) {
      const diffBetweenCircleAndTop = -this.#POINT.y + this.radius;
      this.#POINT.move(new Velocity2D(0, diffBetweenCircleAndTop));
  ...
  moveToDown(): void {
    ...
    if (this.#willOverBilliardsTableBottom()) {
      const diffBetweenCircleAndBottom =
        this.#BILLIARDS_TABLE.height - (this.#POINT.y + this.#RADIUS);
      this.#POINT.move(new Velocity2D(0, diffBetweenCircleAndBottom));
